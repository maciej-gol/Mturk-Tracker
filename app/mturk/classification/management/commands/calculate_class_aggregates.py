...
class CalculateClassAggregates(BaseCommand):
    ...
    option_list = BaseCommand.option_list + (
        make_option("--clear-all", dest="clear_all", action="store_true",
                    default=False),
        make_option("--clear-existing", dest="clear_existing", action="store_true",
                    default=False),
    ...
    def handle(self, *args, **options):
        ...
        # TODO pid = Pid("mturk_classification_aggreagate")
        ...
        if begin is None:
            ...
            if start_time is not None:
                ...
                logger.warn("No crawls to processing")
                return

        begin = begin.replace(tzinfo=None)
        end = end.replace(tzinfo=None)

        queryset = HitGroupClassAggregate.objects.filter(start_time__gte=begin, start_time__lt=end)
